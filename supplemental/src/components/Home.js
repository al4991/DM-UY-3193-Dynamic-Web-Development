...
class Home extends Component {
    ...
    async handleUploadSuccess(filename){
        try { 
            ...
            firebase.firestore().collection('pictures').doc(docRef.id)
            .onSnapshot( (docSnapshot) => {
                console.log('YOYOYOYOYOYOY', docSnapshot.data()); 
                if (docSnapshot.data().labels) { 
                    const cleanLabels = docSnapshot.data().labels.filter(
                        (elem) => 
                            elem !== 'Food' &&
                            elem !== 'Dish' &&
                            elem !== 'Ingredient'
    ...
    search() { 
        ...
        axios.get(`${'https://cors-anywhere.herokuapp.com/'}https://api.yelp.com/v3/businesses/search?location=${'New York City'}&term=${this.state.selectedLabel}`, {
            headers: {
    ...
    renderUploadButton() { 
        if (this.state.docId === '') {
            return (
                <div > 
                    <h1> wut? where? </h1>
                    <label style={{backgroundColor: 'steelblue', color: 'white', padding: 10, borderRadius: 4, cursor: 'pointer', margin:50}}>                    
    ...
    renderYelpButton() { 
        if (this.state.notSearching && this.state.selectedLabel !== '' && this.state.yelpLocations.length === 0) { 
            return (
                <div > 
                    ...
                    <h2> I found these places! </h2>
                    {
                       cleanArray.map(elem => 
                            <div> 
                                <h4>{elem.name}</h4>
                                <p>{elem.address1}</p>
                                <p>{elem.city}, {elem.state} {elem.zipCode}</p>
                            </div> 
                        )
                    }
                </div>
        ...
        if (this.state.labels.length > 0) { 
            return(
                <div> 
                    ...
                    {this.state.labels && this.state.labels.map((elem,i) => <LabelButton labelName={elem} onClick={() => this.selectLabel(i)} />)}
                </div>
            )
        }
    }
    ...
    renderUploadedImage() { 
        if (this.state.image !== '') {
            return (
                <img style={styles.uploadedImage} src={this.state.image} alt=''/>
            )
        }
    }
